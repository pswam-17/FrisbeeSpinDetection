  ...
  if(timer>0){
    ...
    if(windDetect == 0){
      ...
      if(sampleSensors() && Serial.availableForWrite()){
        ...
        if(derivative_mag > 25 || derivative_mag < -25){  // detects winding up motion 
          motionDetect = 1;
          previous_derivative = derivative_mag;
          windDetect = 1;
        }
      }
    }
      ...
      if(angle_state == 0){ // to calculate the instant angle of throw (using tilt angle calculation)
        ...
        angle_display = tilt_angle_degrees;

        before = millis();  // for max spin calculation
        ...
        if(sampleSensors() && Serial.availableForWrite()){
          ...
          if(derivative_mag <= -previous_derivative){  // detects throw motion 
            if(spin_rpm_z < 0){
            ...
            if(sampling > 0){
              ...
              if(cd - cd1 >= 1000){//check sampling time
                sampling = sampling - 1;
                cd1 = cd;
